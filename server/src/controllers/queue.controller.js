...
const sendMail = async (data, token) => {
  try {
    ...
    const featuresHTML = `<ul style="list-style: none">${features
      .split("\n")
      .map((feature) => `<li style="list-style: none">${feature}</li>`)
      .join("")}</ul>`;
    const benefitsHTML = `<ul style="list-style: none">${benefits
      .split("\n")
      .map((feature) => `<li style="list-style: none">${feature}</li>`)
      .join("")}</ul>`;
